...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    if ($conn->query($delete_sql) === TRUE) {
        echo "<script>alert('Product deleted successfully'); window.location.href = 'invcheck.php';</script>";
    } else {
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                        <button onclick=\"window.location.href='editprod_checker.php?id=" . $row["inventory_id"] . "'\" style=\"margin-right: 0px; padding: 3px 9px; font-weight: bold; border-radius: 4px; background-color: #F59607; color: #ffffff; border: none;\">
